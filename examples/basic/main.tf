...
locals {
  ...
  # This can be 1 control plane, 2 workers
  min_size     = 3
  max_size     = 3
  desired_size = 3
...
# aws ec2 describe-instances --region us-east-1 --filters "Name=tag:selector,Values=usernetes-selector" | jq .Reservations[].Instances[].NetworkInterfaces[].PrivateIpAddress
# aws ec2 describe-instances --region us-east-1 --filters "Name=tag:selector,Values=usernetes-selector" | jq .Reservations[].Instances[].NetworkInterfaces[].PrivateIpAddresses[].PrivateDnsName
...
resource "aws_security_group" "security_group" {
  ...
  ingress {
    description = "Allow http on port 8050 for kubeadm"
    from_port   = 6443
    to_port     = 6443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
